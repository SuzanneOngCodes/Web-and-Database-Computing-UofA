...
public class MapReduceQ41 extends Configured implements Tool {
	   public static void main(String[] args) throws Exception {
	      System.out.println(Arrays.toString(args));
	      // Run the configuration
	      int res = ToolRunner.run(new Configuration(), new MapReduceQ41(), args);
	   ...
	   public int run(String[] args) throws Exception {
	      System.out.println(Arrays.toString(args));
	      Job job = new Job(getConf(), "MapReduce1");
	      job.setJarByClass(MapReduceQ41.class);
	      ...
	      public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
	    	...
	    	// Remove all punctuation and set to lower case
	    	  ...
	    	  while (token.hasMoreTokens()){
	    		  int wordLength = token.nextToken().length() ; 
	    		  word.set(Integer.toString(wordLength));
